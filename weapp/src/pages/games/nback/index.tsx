  ...
  if (screen === 'menu') return (
    <ScrollView style={{ flex: 1, backgroundColor: '#F5F7FA' }} scrollY enableFlex>
      <View style={{ padding: '40px 32px 120px', alignItems: 'center' }}>
        ...
        <Text style={{ fontSize: 24, fontWeight: 700, color: '#1A1A2E', marginBottom: 8, wordBreak: 'break-all'}}>{t('nback.title', 'N-Back测试')}</Text>
        <Text style={{ fontSize: 18, color: '#64748B', textAlign: 'center', lineHeight: 1.6, marginBottom: 40, wordBreak: 'break-all'}}>{t('nback.description', '记住N步前的位置和字母')}</Text>
        <Text style={{ fontSize: 16, color: '#94A3B8', marginBottom: 16, wordBreak: 'break-all'}}>{t('nback.chooseLevel', '选择难度')}</Text>
        <View style={{ display: 'flex', flexDirection: 'row', gap: 12, marginBottom: 32, width: '100%' }}>
          {[1, 2, 3, 4].map((n) => (
            <View key={n} onClick={() => startGame(n)} style={{ flex: 1, padding: '20px 0', borderRadius: 14, backgroundColor: nLevel === n ? '#7C3AED' : '#F1F5F9', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <Text style={{ fontSize: 18, fontWeight: 600, color: nLevel === n ? '#FFFFFF' : '#475569', wordBreak: 'break-all'}}>{n}-Back</Text>
            </View>
          ))}
        </View>
      </View>
  ...
  if (screen === 'playing' && trial) return (
    <View style={{ flex: 1, backgroundColor: '#FFFFFF' }}>
      <View style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', padding: '12px 20px', paddingTop: navHeight }}>
        ...
        <Text style={{ fontSize: 16, color: '#94A3B8', wordBreak: 'break-all'}}>{t('nback.trial', '试次 {{current}}/{{total}}', { current: trialIndex + 1, total: trials.length })} · {nLevel}-Back</Text>
        <View style={{ width: 60 }} />
      </View>
      <View style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '0 32px' }}>
              ...
              {Array.from({ length: GRID_SIZE }).map((_, col) => {
                ...
                return (
                  <View key={col} style={{ flex: 1, margin: 3, borderRadius: 12, backgroundColor: isActive ? '#7C3AED' : '#F3F4F6', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    {isActive && <Text style={{ fontSize: 24, fontWeight: 700, color: '#FFFFFF', wordBreak: 'break-all'}}>{trial.letter}</Text>}
                  </View>
                );
              })}
            </View>
        ...
        {showStimulus && <Text style={{ fontSize: 16, color: '#94A3B8', marginBottom: 32, wordBreak: 'break-all'}}>{t('nback.hearLetter', '字母: {{letter}}', { letter: trial.letter })}</Text>}
        <View style={{ display: 'flex', flexDirection: 'row', gap: 16, width: '100%' }}>
          <View onClick={handlePositionPress} style={{ flex: 1, padding: '24px 0', borderRadius: 16, backgroundColor: posPressed ? '#22C55E' : '#EDE9FE', border: posPressed ? 'none' : '2px solid #A78BFA', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <Text style={{ fontSize: 18, fontWeight: 700, color: posPressed ? '#FFFFFF' : '#7C3AED', wordBreak: 'break-all'}}>{t('nback.position', '位置')}</Text>
          </View>
          <View onClick={handleSoundPress} style={{ flex: 1, padding: '24px 0', borderRadius: 16, backgroundColor: soundPressed ? '#22C55E' : '#E0E7FF', border: soundPressed ? 'none' : '2px solid #818CF8', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <Text style={{ fontSize: 18, fontWeight: 700, color: soundPressed ? '#FFFFFF' : '#4F46E5', wordBreak: 'break-all'}}>{t('nback.sound', '声音')}</Text>
          </View>
        </View>
      </View>
      {screen === 'results' && (() => { const acc = getAccuracy(trials, responses); return (
        <View style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <View style={{ backgroundColor: '#FFFFFF', borderRadius: 24, padding: '40px 32px', alignItems: 'center', width: '80%' }}>
            <Text style={{ fontSize: 22, fontWeight: 700, color: '#1A1A2E', marginBottom: 24, wordBreak: 'break-all'}}>{t('nback.roundComplete', '回合完成')}</Text>
            <View style={{ display: 'flex', flexDirection: 'row', gap: 16, marginBottom: 24, width: '100%' }}>
              <View style={{ flex: 1, backgroundColor: '#F8FAFC', borderRadius: 14, padding: '20px 12px', alignItems: 'center' }}><Text style={{ fontSize: 22, fontWeight: 700, color: '#7C3AED', wordBreak: 'break-all'}}>{acc.positionAcc}%</Text><Text style={{ fontSize: 14, color: '#94A3B8', wordBreak: 'break-all'}}>{t('nback.positionAcc', '位置')}</Text></View>
              <View style={{ flex: 1, backgroundColor: '#F8FAFC', borderRadius: 14, padding: '20px 12px', alignItems: 'center' }}><Text style={{ fontSize: 22, fontWeight: 700, color: '#4F46E5', wordBreak: 'break-all'}}>{acc.soundAcc}%</Text><Text style={{ fontSize: 14, color: '#94A3B8', wordBreak: 'break-all'}}>{t('nback.soundAcc', '声音')}</Text></View>
              <View style={{ flex: 1, backgroundColor: '#F8FAFC', borderRadius: 14, padding: '20px 12px', alignItems: 'center' }}><Text style={{ fontSize: 22, fontWeight: 700, color: '#10B981', wordBreak: 'break-all'}}>{acc.overall}%</Text><Text style={{ fontSize: 14, color: '#94A3B8', wordBreak: 'break-all'}}>{t('nback.overall', '综合')}</Text></View>
            </View>
            <Text style={{ fontSize: 16, color: '#64748B', marginBottom: 24, wordBreak: 'break-all'}}>{t('nback.nextLevel', '建议难度: {{next}}', { next: getNBackLevel(nLevel, acc.overall) })}</Text>
            <View onClick={() => startGame()} style={{ backgroundColor: '#7C3AED', borderRadius: 16, padding: '20px 40px', width: '100%', alignItems: 'center' }}><Text style={{ fontSize: 18, fontWeight: 600, color: '#FFFFFF', wordBreak: 'break-all'}}>{t('nback.playAgain', '再来一次')}</Text></View>
          </View>
        </View>
      ); })()}
    </View>
